...
import { GET_BOOKS, GET_COMMENTS, GET_MEMBER } from '../../apollo/user/query';
...
const AgentDetail: NextPage = ({ initialInput, initialComment, ...props }: any) => {
	...
	const {
		...
		onCompleted: (data: T) => {
			...
			setBookTotal(data?.getBooks?.metaCounter[0]?.total ?? 0);
		},
	});
	...
	useEffect(() => {
		if (searchFilter.search.memberId) {
			getBooksRefetch({ variables: { input: searchFilter } }).then();
		}
	}, [searchFilter]);
	useEffect(() => {
		if (commentInquiry.search.commentRefId) {
			getBooksRefetch({ variables: { input: commentInquiry } }).then();
		}
	}, [commentInquiry]);
	...
	const bookPaginationChangeHandler = async (event: ChangeEvent<unknown>, value: number) => {
	...
	const likeBookHandler = async (user: any, id: string) => {
		try {
			if (!id) return;
			if (!user._id) throw new Error(Messages.error2);
					...
					<Stack className={'agent-home-list'}>
						<Stack className={'card-wrap'}>
							{agentBooks.map((book: Book) => {
								return (
									<div className={'wrap-main'} key={book?._id}>
								...
								<>
									<Stack className="pagination-box">
										<Pagination
											...
											count={Math.ceil(bookTotal / searchFilter.limit) || 1}
											onChange={bookPaginationChangeHandler}
											shape="circular"
											color="primary"
										/>
									</Stack>
									<span>
										Total {bookTotal} book{bookTotal > 1 ? 'ies' : 'y'} available
									</span>
								</>
							) : (
