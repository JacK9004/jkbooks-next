...
const CommunityDetail: NextPage = ({ initialInput, ...props }: T) => {
	...
	const likeBodArticleHandler = async (user: any, id: any) => {
		try {
										...
										<Stack className="info">
											<Stack className="icon-info">
												{boardArticle?.meLiked && boardArticle?.meLiked[0]?.myFavorite ? (
													<ThumbUpAltIcon onClick={() => likeBodArticleHandler(user, boardArticle?._id)} />
													) : (
													<ThumbUpOffAltIcon onClick={() => likeBodArticleHandler(user, boardArticle?._id)} />
												)}
